...
with open('F:/PosDoc Projects/11_Industrial Projects/HuaWei/WISF/R_Results/Benchmarking/' + "PSSE Frequency Control.csv", 'rb') as t_file:
...
#[TIME, FREQ, V_INV, V_POC, P_INV, P_POC, Q_INV, Q_POC]),
# with open('F:/PosDoc Projects/11_Industrial Projects/HuaWei/WISF/R_Results/Benchmarking/' + "PSCAD-Contingency-deep.csv", 'rb') as t_file:
#     FileData = csv.reader(t_file)
#     FileData = list(FileData)
...
for t in range(4, 65, 61):
    ...
    TIME_PSSE_start_index = (numpy.abs(time_psse - t)).argmin()
    TIME_PSSE_end_index = (numpy.abs(time_psse - (t + 5))).argmin()
    ...
    GraphPath = 'F:/PosDoc Projects/11_Industrial Projects/HuaWei/WISF/R_Results/Benchmarking/'
    ...
    CurrentAx[0][0].set_xlim([t, t + 60])
    CurrentAx[1][0].set_xlim([t, t + 60])
    CurrentAx[2][0].set_xlim([t, t + 60])
    CurrentAx[0][1].set_xlim([t, t + 60])
    CurrentAx[1][1].set_xlim([t, t + 60])
    CurrentAx[2][1].set_xlim([t, t + 60])

    CurrentAx[0][0].set_ylim([0.0, 1.3])
    CurrentAx[0][1].set_ylim([0.0, 1.3])
    ...
    save_figure_name = GraphPath + str(t) + 'frequency.png'
