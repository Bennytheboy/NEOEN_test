...
ClauseName="Benchmarking_droop_2"
...
pp_g=numpy.arange(0, 0.1, 0.002)
i_g= numpy.arange(0, 1, 0.05)
# Initialize
for i in range(0, len(pp_g)):
    for j in range(0, len(i_g)):
        ...
        psspy.change_con(var_ppc_conp+1, pp_g[i])  #Proportional gain of reactive power PI controller 0.001 0.05
        psspy.change_con(var_ppc_conp+2, i_g[j])  # Integral gain of reactive power PI controller 0.015  0.4
        psspy.change_con(var_ppc_conp+23, 0.5)  # Integral gain of reactive power PI controller 0.015  0.4
        ...
        save_figure_name = GraphPath + "/" + ClauseName + '-'+str(pp_g[i])+'_' + str(i_g[j])+'_' + '.png'
