...
for i in range(0, 1):
    for fault_type in range(1, 5):
        ...
        if fault_type == 2:  # single phase
            psspy.dist_scmu_fault_2([0, 0, 1, 700, _i], [0.0001, 0.0001, 0.0, 0.0])
            fault_name = 'SinglePhase'
            fault_time = 0.720

        if fault_type == 3:  # phase to phase
            psspy.dist_scmu_fault_2([0, 0, 2, 700, _i], [0.0001, 0.0001, 999.0, 999.0])
            fault_name = 'TwoPhase'
            fault_time = 0.720

        if fault_type == 4:  # phase to phase to ground
            psspy.dist_scmu_fault_2([0, 0, 2, 700, _i], [0.0001, 0.0, 0.0001, 0.0])
...
GraphPath = FigurePath + ClauseName + '/' + 'Contingency' + '/'
...
numpy.savetxt(GraphPath+'PSSE Fault.csv', numpy.transpose([TIME,FREQ,V_INV,V_POC,P_INV,P_POC,Q_INV,Q_POC]), delimiter=',')
...
save_figure_name = GraphPath + 'Contingency-' + '.png'
