...
psspy.load_data_3(600, r"""1""", [_i, _i, _i, _i, _i], [885, 20, _f, _f, _f, _f])  #0.08pu/s  to -0.02pu
psspy.run(0, 1.25, 1000, 1, 0)
psspy.load_data_3(600, r"""1""", [_i, _i, _i, _i, _i], [999, 20, _f, _f, _f, _f])  #0.02pu/s  to -0.04pu
...
CurrentFig, CurrentAx = plt.subplots(4, 2, sharex=False, figsize=(20, 15));
...
CurrentAx[3][0].plot(chandata['time'], chandata[1]);
...
CurrentAx[3][0].tick_params(axis='both', which='both', labelsize=24)
...
CurrentAx[3][0].set_xlim(left=0)

CurrentAx[0][0].set_ylim([0, 1.3])
CurrentAx[1][0].set_ylim([-0.4, 0.4])
CurrentAx[2][0].set_ylim([0.85, 1.5])
CurrentAx[0][1].set_ylim([-5, 550])
CurrentAx[1][1].set_ylim([-300, 300])
CurrentAx[2][1].set_ylim([0.9, 1.1])
CurrentAx[3][0].set_ylim([-0.05, 0.01])
...
CurrentAx[3][0].set_xlabel(r"""Time/s""")
...
CurrentAx[3][0].set_ylabel(r"""delta_f/PU""")
...
CurrentAx[3][0].set_title(r"""System frequency""")
...
F_sys_data = numpy.array(chandata[1]);
ResultFile = open(GraphPath + "/" + testName +  '-' + '.csv', 'w');
ResultFile.write('time, P_gen, Q_gen, inverter_V, P_injection, Q_injection, POC_V, system_f')
ResultFile.write('\n')
for t in range(0, len(t_data)):
    ResultFile.write(
        str(t_data[t]) + ',' + str(p_gen_data[t]) + ','+ str(q_gen_data[t]) + ',' +
        str(V_inv_data[t]) + ',' + str(P_Poc_data[t]) + ',' + str(Q_Poc_data[t]) + ',' + str(V_poc_data[t]) + ',' + str(F_sys_data[t]) + ',' )
    ResultFile.write('\n')
ResultFile.close()
